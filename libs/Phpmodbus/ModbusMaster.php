...
class ModbusMaster {
   ...
   */         
  function __construct($host, $protocol, $port="502"){
  ...
  private function readCoilsPacketBuilder($unitId, $reference, $quantity){
    ...
    // build body
    $buffer2 = "";
    $buffer2 .= (new iecType)->iecBYTE(1);              // FC 1 = 1(0x01)
    // build body - read section    
    $buffer2 .= (new iecType)->iecINT($reference);      // refnumber = 12288      
    $buffer2 .= (new iecType)->iecINT($quantity);       // quantity
    $dataLen += 5;
    // build header
    $buffer3 = '';
    $buffer3 .= (new iecType)->iecINT(rand(0,65000));   // transaction ID
    $buffer3 .= (new iecType)->iecINT(0);               // protocol ID
    $buffer3 .= (new iecType)->iecINT($dataLen + 1);    // lenght
    $buffer3 .= (new iecType)->iecBYTE($unitId);        //unit ID
  ...
  private function readInputDiscretesPacketBuilder($unitId, $reference, $quantity){
    ...
    // build body
    $buffer2 = "";
    $buffer2 .= (new iecType)->iecBYTE(2);              // FC 2 = 2(0x02)
    // build body - read section    
    $buffer2 .= (new iecType)->iecINT($reference);      // refnumber = 12288      
    $buffer2 .= (new iecType)->iecINT($quantity);       // quantity
    $dataLen += 5;
  ...
  private function readMultipleRegistersPacketBuilder($unitId, $reference, $quantity){
    ...
    // build body
    $buffer2 = "";
    $buffer2 .= (new iecType)->iecBYTE(3);             // FC 3 = 3(0x03)
    // build body - read section    
    $buffer2 .= (new iecType)->iecINT($reference);  // refnumber = 12288      
    $buffer2 .= (new iecType)->iecINT($quantity);       // quantity
    $dataLen += 5;
  ...
  private function writeMultipleRegisterPacketBuilder($unitId, $reference, $data, $dataTypes, $swapregs){
    ...
    foreach($data as $key=>$dataitem) {
      if($dataTypes[$key]=="INT"){
        $buffer1 .= (new iecType)->iecINT($dataitem);   // register values x 
        $dataLen += 2;
      }
      elseif($dataTypes[$key]=="DINT"){
        $buffer1 .= (new iecType)->iecDINT($dataitem, $swapregs, $this->endianness);   // register values x 
        $dataLen += 4;
      }
      elseif($dataTypes[$key]=="REAL") {
        $buffer1 .= (new iecType)->iecREAL($dataitem, $swapregs, $this->endianness);   // register values x 
        $dataLen += 4;
      }       
      else{
        $buffer1 .= (new iecType)->iecINT($dataitem);   // register values x 
    ...
    $buffer2 .= (new iecType)->iecINT($reference);      // refnumber = 12288      
    $buffer2 .= (new iecType)->iecINT($dataLen/2);      // word count      
    $buffer2 .= (new iecType)->iecBYTE($dataLen);     	// byte count
    $dataLen += 6;
    // build header
    $buffer3 = '';
    $buffer3 .= (new iecType)->iecINT(rand(0,65000));   // transaction ID    
